...
public interface GitlabService {
    ...
    default List<PullRequest> allPullRequests(String owner, String repo) throws IOException {
        final String id = owner + "/" + repo; // this will be URI encoded by OKHttp
        final Response<List<PullRequest>> initial = pullRequests(id).execute();
        ...
        while (pg != null && pg.hasNext()) {
            final Response<List<PullRequest>> nextPage = pullRequests(id, pg.next()).execute();
