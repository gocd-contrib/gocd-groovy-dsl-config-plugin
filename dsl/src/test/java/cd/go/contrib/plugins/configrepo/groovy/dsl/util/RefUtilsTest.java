...
import static cd.go.contrib.plugins.configrepo.groovy.dsl.util.RefUtils.sanitizeRef;
...
class RefUtilsTest {
    ...
    @Test
    void sanitizeRefMakesRefSafeForIdentifiers() {
        assertEquals("h_ppy_n0t___d", sanitizeRef("h@ppy.n0t.$@d"));
        assertEquals("-but-dashes--are-ok", sanitizeRef("-but-dashes--are-ok"));
    }
}