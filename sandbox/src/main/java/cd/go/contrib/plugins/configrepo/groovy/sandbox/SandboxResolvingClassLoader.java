 ...
 * given to {@link GroovyScriptEngine} via the constructor.
...
class SandboxResolvingClassLoader extends ClassLoader {
    ...
    private static final LoadingCache<ClassLoader, LoadingCache<String, java.util.Optional<Class>>> parentClassCache = makeParentCache(new CacheFunction<java.util.Optional<Class>>() {
        @Override
        public java.util.Optional<Class> compute(ClassLoader parentLoader, String name) {
            try {
    ...
    private static final LoadingCache<ClassLoader, LoadingCache<String, java.util.Optional<URL>>> parentResourceCache = makeParentCache(new CacheFunction<java.util.Optional<URL>>() {
        @Override
        public java.util.Optional<URL> compute(ClassLoader parentLoader, String name) {
            return java.util.Optional.ofNullable(parentLoader.getResource(name));
        }
    });
    ...
    protected synchronized Class<?> loadClass(String name, boolean resolve) throws ClassNotFoundException {
        if (name.startsWith("org.kohsuke.groovy.sandbox.")) {
            ...
            Class<?> c = parentClassCache.getUnchecked(getParent()).getUnchecked(name).orElse(null);
            if (c != null) {
                if (resolve) {
    ...
    public URL getResource(String name) {
        return parentResourceCache.getUnchecked(getParent()).getUnchecked(name).orElse(null);
    }
